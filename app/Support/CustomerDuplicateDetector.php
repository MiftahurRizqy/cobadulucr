...
class CustomerDuplicateDetector
{
    public function detect(User $user, array $input, ?Customer $exceptCustomer = null, ?Lead $exceptLead = null): Collection
    {
        ...
        $customers = Customer::query()->visibleTo($user)
            ...
            ->get(['id', 'customer_id', 'company_name', 'phone', 'email', 'npwp']);
        $leads = $user->canAccess('leads.view')
            ? Lead::query()->visibleTo($user)->where('status', '!=', 'converted')
                ...
                ->get(['id', 'lead_id', 'company_name', 'phone', 'email'])
            : collect();
        ...
        return $customers->toBase()
            ->map(fn ($record) => $this->match($record, 'Customer', $phone, $email, $npwp, $company))
            ->concat($leads->toBase()->map(fn ($record) => $this->match($record, 'Lead', $phone, $email, null, $company)))
            ->filter()
            ->sortByDesc('score')
            ->take(8)
            ->values();
    ...
    private function match($record, string $type, string $phone, string $email, ?string $npwp, string $company): ?array
    {
        $reasons = [];
        $score = 0;
