...
@section('eyebrow','Administrasi · Area & cabang')
@section('content')
<form method="POST" action="{{ $area->exists ? route('areas.update',$area) : route('areas.store') }}" class="mx-auto max-w-3xl">
    ...
    <section class="card overflow-hidden">
        <div class="border-b border-slate-100 px-5 py-4"><h3 class="section-title">Informasi area</h3><p class="mt-1 text-[11px] text-slate-400">Area digunakan untuk membatasi cakupan customer, lead, dan pengguna.</p></div>
        <div class="grid gap-5 p-5 md:grid-cols-2">
            ...
            <div><label class="label">Cabang utama</label><input class="field" name="branch" value="{{ old('branch',$area->branch) }}" placeholder="Contoh: Jakarta"></div>
