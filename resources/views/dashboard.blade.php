...
<div class="mt-5 grid gap-5 xl:grid-cols-2">
    ...
    <section class="card overflow-hidden">
        ...
        <div class="divide-y divide-slate-100">@forelse($recentActivities as $activity)<div class="flex items-center gap-3 px-5 py-3.5"><div class="grid size-8 shrink-0 place-items-center rounded-lg bg-slate-100 text-[10px] font-black uppercase text-slate-500">{{ mb_substr($activity->type,0,2) }}</div><div class="min-w-0 flex-1"><div class="truncate text-xs font-bold text-ink">{{ $activity->summary }}</div><div class="mt-1 truncate text-[10px] text-slate-400">{{ $activity->subject_name }} · {{ $activity->user->name }}</div></div><span class="shrink-0 text-[9px] font-semibold text-slate-400">{{ $activity->occurred_at->diffForHumans(null,true) }}</span></div>@empty<div class="empty-state">Belum ada aktivitas.</div>@endforelse</div>
    </section>
</div>
@endsection
