...
<div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_420px]">
    <section class="card overflow-hidden">
        <div class="border-b border-slate-100 px-5 py-4"><h3 class="section-title">Daftar perusahaan</h3><p class="mt-1 text-xs text-slate-400">Setiap perusahaan menggunakan database CRM tersendiri.</p></div>
        <div class="divide-y divide-slate-100">
            ...
            <div x-data="{ editing: false, completing: false }" class="flex flex-wrap items-center gap-4 px-5 py-4">
                ...
                <div class="min-w-0 flex-1"><div class="truncate text-sm font-extrabold text-ink">{{ $company->name }}</div><div class="mt-1 truncate text-[10px] text-slate-400">CRM perusahaan · Database terpisah</div></div>
    ...
    <form method="POST" action="{{ route('tenants.store') }}" enctype="multipart/form-data" class="card p-6">@csrf
        <h3 class="section-title">Tambah perusahaan</h3><p class="mt-1 text-xs text-slate-400">Kode sistem, database terpisah, dan akun administrator dibuat otomatis.</p>
        ...
        <button class="btn-primary mt-5 w-full">Buat perusahaan dan database</button>
    </form>
</div>
@endsection
