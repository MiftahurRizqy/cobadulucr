            ...
            @forelse($activities as $activity)
                ...
                <tbody id="activity-{{ $activity->id }}" data-activity-evidence-scope x-data="{ modalOpen: @js((int) request('activity') === $activity->id), inviteOpen: false, openModal() { this.modalOpen = true; document.body.style.overflow = 'hidden' }, closeModal() { this.modalOpen = false; this.inviteOpen = false; document.body.style.overflow = '' } }" x-init="if (modalOpen) document.body.style.overflow = 'hidden'" class="border-b border-slate-200 last:border-b-0">
                    <tr class="activity-row group transition odd:bg-white hover:bg-indigo-50/50" :class="modalOpen&&'bg-indigo-50/60'">
                        ...
                        <td class="border-r border-slate-100 px-3 py-3 align-top"><div class="truncate text-[12px] font-bold text-slate-800" title="{{ $activity->customer?->company_name ?? $activity->lead?->company_name }}">{{ $activity->customer?->company_name ?? $activity->lead?->company_name ?? 'Data tidak tersedia' }}</div>@if($activity->lead)<div class="mt-1 text-[10px] text-amber-600">Lead</div>@elseif($activity->opportunity)<div class="mt-1 truncate text-[10px] text-brand-600" title="{{ $activity->opportunity->title }}">{{ $activity->opportunity->title }}</div>@else<div class="mt-1 text-[10px] text-slate-400">Aktivitas umum</div>@endif</td>
