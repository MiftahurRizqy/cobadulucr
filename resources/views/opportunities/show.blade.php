        ...
        <section id="daftar-produk" class="card scroll-mt-24 overflow-hidden" x-data="{ addProduct: false, editing: null, historyOpen: false }">
            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[920px] text-left text-xs">
                    <thead class="table-head"><tr><th class="px-5 py-3">Produk</th><th class="px-4 py-3">Jenis</th><th class="px-4 py-3 text-right">Est. Qty/Bulan</th><th class="px-4 py-3 text-right">Target Harga (UOM)</th><th class="px-4 py-3 text-right">Harga Penawaran (UOM)</th><th class="px-4 py-3 text-right">Subtotal</th><th class="px-5 py-3">Status barang</th><th class="w-20 px-4 py-3 text-center">Aksi</th></tr></thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($opportunity->items as $item)
                            <tr x-data="{ editPrice: false }">
                                ...
                                <td class="px-4 py-3.5"><span class="rounded-full px-2 py-1 text-[10px] font-bold {{ $item->product_type === 'custom' ? 'bg-violet-50 text-violet-700' : 'bg-sky-50 text-sky-700' }}">{{ \App\Models\OpportunityItem::PRODUCT_TYPES[$item->product_type ?? 'regular'] ?? 'Reguler' }}</span></td>
                        ...
                        @empty
                            <tr><td colspan="8" class="p-8 text-center text-slate-400">Belum ada rincian produk.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            ...
            @endphp
            @if($customItems->isNotEmpty() && $customProgress->stages() && ! $opportunity->pipeline->uses_pipeline_for_custom_progress)
                <section class="border-t border-slate-100 bg-violet-50/30 px-5 py-4">
                    <div class="mb-3"><h4 class="text-xs font-extrabold text-ink">Pekerjaan produk Custom</h4><p class="mt-1 text-[10px] text-slate-500">Progres internal per produk. Pipeline di atas tetap menunjukkan progres order dengan customer.</p></div>
                    <div class="space-y-2">@foreach($customItems as $item)<div class="flex items-center justify-between gap-4 rounded-xl border border-violet-100 bg-white px-3 py-3"><span class="text-xs font-bold text-ink">{{ $item->product_name }}</span><form method="POST" action="{{ route('opportunities.items.custom-stage', [$opportunity, $item]) }}">@csrf @method('PATCH')<select name="custom_stage" class="field !h-9 !min-w-40 !py-1.5 text-xs font-bold text-violet-700" onchange="this.form.submit()">@foreach($customProgress->stages() as $stage)<option value="{{ $stage['key'] }}" @selected((string) $item->custom_stage === $stage['key'])>{{ $stage['name'] }}</option>@endforeach</select></form></div>@endforeach</div>
                </section>
            @endif
            ...
            <div x-show="addProduct" x-cloak x-transition.opacity @keydown.escape.window="addProduct=false" @click.self="addProduct=false" class="fixed inset-0 z-[120] grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm">
                <form method="POST" action="{{ route('opportunities.items.store', $opportunity) }}" enctype="multipart/form-data" x-data="{ photoPreview: null, previewPhoto(event) { if (this.photoPreview) URL.revokeObjectURL(this.photoPreview); this.photoPreview = event.target.files[0] ? URL.createObjectURL(event.target.files[0]) : null } }" class="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
                    ...
                    <div class="grid gap-4 p-5 sm:grid-cols-2">
                        <div class="sm:col-span-2"><label class="label">Nama produk *</label><input name="product_name" class="field" placeholder="Contoh: Paper bowl 12 oz" required></div><div><label class="label">Industri *</label><select name="market_segment" class="field" required><option value="drink">Drink</option><option value="food">Food</option><option value="industry">Industri</option></select></div><div><label class="label">Jenis produk *</label><select name="product_type" class="field"><option value="regular">Reguler</option><option value="custom">Custom</option></select></div>
                        ...
                        <div><label class="label">Target Harga (UOM)</label><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span><input name="target_price" data-money inputmode="numeric" class="field !pl-9" placeholder="0"></div></div>
                        <div><label class="label">Harga Penawaran (UOM)</label><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-xs font-semibold text-slate-400">Rp</span><input name="unit_price" data-money inputmode="numeric" class="field !pl-9" placeholder="0"></div><p class="mt-1 text-[10px] text-slate-400">Isi harga untuk 1 satuan sesuai UOM produk, bukan harga total.</p></div>
            ...
            @foreach($opportunity->items as $item)
                <div x-show="editing === {{ $item->id }}" x-cloak x-transition.opacity @keydown.escape.window="editing=null" @click.self="editing=null" class="fixed inset-0 z-[120] grid place-items-center bg-slate-950/60 p-4 backdrop-blur-sm">
                    <form method="POST" action="{{ route('opportunities.items.update', [$opportunity, $item]) }}" enctype="multipart/form-data" x-data="{ photoPreview: @js($item->photo_path ? Storage::disk('public')->url($item->photo_path) : null), previewPhoto(event) { this.photoPreview = event.target.files[0] ? URL.createObjectURL(event.target.files[0]) : null } }" class="w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">@csrf @method('PATCH')
                        ...
                        <div class="grid gap-4 p-5 sm:grid-cols-2">
                            <div class="sm:col-span-2"><label class="label">Nama produk *</label><input name="product_name" class="field" value="{{ $item->product_name }}" required></div><div><label class="label">Industri *</label><select name="market_segment" class="field" required><option value="drink" @selected($item->market_segment==='drink')>Drink</option><option value="food" @selected($item->market_segment==='food')>Food</option><option value="industry" @selected($item->market_segment==='industry')>Industri</option></select></div><div><label class="label">Jenis produk *</label><select name="product_type" class="field"><option value="regular" @selected(($item->product_type ?? 'regular') === 'regular')>Reguler</option><option value="custom" @selected($item->product_type === 'custom')>Custom</option></select></div>
                        ...
                        @foreach($opportunity->items as $item)
                            <div class="grid items-end gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4 sm:grid-cols-[1fr_160px]">
                                <div>
                                    ...
                                    <div class="mt-1 text-[10px] text-slate-400">{{ number_format($item->quantity,0,',','.') }} {{ \App\Models\Opportunity::QUANTITY_UNITS[$item->quantity_unit] ?? ucfirst($item->quantity_unit) }} · Target Harga (UOM) {{ $item->target_price !== null ? 'Rp '.number_format($item->target_price,0,',','.') : 'belum diisi' }}</div>
                                </div>
                                <div>
                                    <input type="hidden" name="items[{{ $loop->index }}][id]" value="{{ $item->id }}">
                                    <label class="label">Harga Penawaran (UOM)</label>
